...
It's a dry run, not an integration. It reads the Notion CSV, cleans the two fields that break things (website into a bare domain, employee range onto a fixed set of bands), then decides what should happen to each row: create, attach to a company already seen in the batch, or hold for a person.

Rows that already carry a `CRM company ID` are skipped, which is what makes running it twice harmless.

Then it writes four files: companies, people, deals, and the review queue. Those are the files you'd feed straight into Attio's importer.

It doesn't talk to Notion or Attio. Nothing leaves the machine it runs on.
